...
%		New parameter checking and organization of functions
...
% 2013-02-21, Cleaned up code, tested on MATLAB Version 8.0.0.783 (R2012b)
% 2013-02-26, Fixed issue with reading contour levels correctly (pos, neg)
% 2016-03-22, Empty lines in parameter files (procs) handled
% Nils Nyberg, Copenhagen University, nn@sund.ku.dk
% 2019-01-16, Function now reads shimvalues into A.Shims
% 2020-12-03, Function adjusted for Matlab2020a (use of perl and reading of
% text files).
% 2020-01-24, Use of matlab.lang.makeValidName to avoid that dynamic field
% names crash the program.

% Nils Nyberg, Bruker Nordic AB, nils.nyberg@bruker.com
...
case 'textfile'
	...
	A = rbnmr(DS,silent);
case 'datafile'
	if isempty(fileparts(D)) % D = '1r' or '2rr', checked for existense
...
fid = fopen('title','r');
if (fid ~= -1)
%         title = textscan(fid,'%s','Whitespace','','EndOfLine','\r\n','ReturnOnError',1);
        title = fgetl(fid);
        fclose(fid);
		if isempty(title)
...
%% Read shims
if exist('../../shimvalues','file') == 2
	A.Shims = readshims('../../shimvalues');
end
...
A = ReadTextFileRowByRow(FileName);
...
while i < length(TypeOfRow)
    ...
    switch TypeOfRow{i,1}
        case 'ParVal'
            LastParameterName = matlab.lang.makeValidName(TypeOfRow{i,2}{1});
...
function S = readshims(FileName)
% READSHIMS	Reads contents of shimvalues-file
%
% SYNTAX:	S = readshims(FileName);
%
% IN:		FileName:	Name of file name to readshims
%
% OUT:		S:		Shims
%

% Nils Nyberg, Bruker, 2019-01-07


if nargin < 1; FileName = '../../shimvalues'; end

if not(exist(FileName,'file')==2)
	sprintf('Shim values not found, no %s.',FileName);
end
...
R = {...
	'^([XYZ].+)\s(-?\d+)',						'Shims';...
	'^(\(X2-Y2\).+)\s(-?\d+)',					'X2-Y2Shims';...
	'^(FIELD)\s+(-?\d+.\d+)',					'Field';...
	'^(LOCK\S+)\s+(-?\d+.\d+)',					'Lock';...
	'^(LOOP\S+)\s+(-?\d+.\d+)',					'Loop';...
	'^SHIM_SETTING \[(\d+\s?)\]\s+(-?\d+.\d+)',	'ShimSetting';...
	'^(.+)',									'Anything'	...
    };
...
while i < length(TypeOfRow)
    i=i+1;
    switch TypeOfRow{i,1}
        case {'Shims','Field','Lock','Loop'}
            LastParameterName = matlab.lang.makeValidName(deblank(TypeOfRow{i,2}{1}));
            S.Shim.(LastParameterName)=str2double(TypeOfRow{i,2}{2});
        case 'X2-Y2Shims'
            LastParameterName = matlab.lang.makeValidName(deblank(regexprep(TypeOfRow{i,2}{1},'\(X2-Y2\)(.+)','X2_Y2_$1')));
            S.Shim.(LastParameterName)=str2double(TypeOfRow{i,2}{2});
        case 'ShimSetting'
            ShimSettingIndex = str2double(TypeOfRow{i,2}{1});
			if ShimSettingIndex == 1
				S.ShimSetting = zeros(40,1);
			end
            S.ShimSetting(ShimSettingIndex)=str2double(TypeOfRow{i,2}{2});
        case {'Empty','Anything'}
            % Do nothing
    end
end

function DS = search_data(pathRE,silent)
...
PerlFile = tempname;
[perlfid,errmsg] = fopen(PerlFile,'wt');
if perlfid == -1
    if ~silent
        sprintf('RBNMR: Temporary Perl-file for searching NMR-files could not be created [%s].\n',errmsg);
    end
    DS = [];
    return
else
...
if ~silent
    disp('Searching for data files with a perl script...');
end

[result, status] = perl(PerlFile);
...
function A = ReadTextFileRowByRow(FileName)
% Read file row by row
FileID = fopen(FileName);
RowCount = 1;
FileRowByRow{RowCount} = fgetl(FileID);
while ischar(FileRowByRow{RowCount})
    RowCount = RowCount + 1;
    FileRowByRow{RowCount} = fgetl(FileID);
end
fclose(FileID);
A = FileRowByRow(1:RowCount-1)';
